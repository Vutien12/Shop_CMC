    ...
    <div class="dt-container form-inline dt-bootstrap dt-empty-footer">
      <div class="row dt-layout-row">
        <div class="dt-layout-cell dt-layout-start col-sm-6">
          <button class="btn btn-default btn-delete" @click="deleteRecords">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="16" viewBox="0 0 14 16" fill="none">
                                ...
                                <path
                                    ...
                                    stroke="#020010" stroke-width="1.5" stroke-linecap="round"></path>></svg>
            <span>{{ $t('buttons.delete') }}</span>
          </button>
        </div>

        <div class="dt-layout-cell dt-layout-end col-sm-6">
          <label for="search">{{ $t('table.search_here') }}</label>
          <input
            id="search"
            v-model="keyword"
            class="form-control input-sm"
            :placeholder="$t('table.search_here')"
            type="search"
          />
        </div>
      </div>
      ...
      <div v-if="loading" class="dt-processing panel panel-default">
        <div>{{ $t('table.loading') }}</div>
      </div>

      <pagination v-if="pagination" :data="pagination" @page-change="fetchData" />
    </div>
  </div>
</template>
