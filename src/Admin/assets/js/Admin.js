...
export default class {
    ...
    confirmationModal() {
        ...
        $("#delete-records").on("click", function() {
            confirmationModal.modal("show");
            const selectedValues = $('.index-table').find(".select-row:checked").map(function() {
                return $(this).val();
            }).filter(id => !isNaN(id)).get();

            confirmationModal.find("form").find('input[name="ids"][type="hidden"]').val(JSON.stringify(selectedValues));
            confirmationModal.find("form").attr('action', Ecommerce.data['route.delete']);
        });
    }
    ...
    searchDatatable () {
        ...
        function handleSearch(event) {
            ...
            const keyword = $(config.selectors.input).val().trim();
            updateURL(keyword);
        }
        ...
        return {
            init: function(options = {}) {
                ...
                $(config.selectors.input).on('keyup',
