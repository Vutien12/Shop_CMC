          ...
          <div v-for="p in products" :key="p.id" class="product-card">
            <div class="product-card-inner">
              <div class="product-badge-row">
                ...
                <button @click="toggleLike(p)" :class="['like-btn', { liked: p.isLiked }]">
...
// Use storeToRefs for reactive state (keeps refs intact when destructuring).
// Actions (functions) are taken directly from the store.
const {
...
const { fetchCategories, fetchProducts, setSort, setPageSize, setPriceRange, toggleCategory, changePage, toggleLike } = store;
...
onMounted(async () => {
  ...
  await fetchProducts();
  await nextTick();
  // Use defensive access in case products is unexpectedly undefined
  console.log('After render - Products:', products?.value ? products.value.length : 0);
});
</script>
