                                ...
                                <div class="form-group-wrapper">
                                    ...
                                    <div class="form-input-wrapper">
                                        ...
                                        >
                                            ...
                                            <option value="4">Drone Specifications</option>
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-input-wrapper">
                                        <div class="field-with-error-tooltip">
                                            ...
                                            <select
                                                ...
                                                style="width: 100%;"
                                            >
                                                <option value="1" :selected="form.categories.includes(1)">Electronics</option>
                                                <option value="2" :selected="form.categories.includes(2)">¦–– Computers</option>
                                                <option value="3" :selected="form.categories.includes(3)">¦–– Cameras</option>
                                                <option value="4" :selected="form.categories.includes(4)">¦–– Drones</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
...
import { getAttribute, createAttribute, updateAttribute } from '@/api/attributeApi.js';
...
export default {
    ...
    setup() {
        ...
        // Category options (can be loaded from API)
        const categories = [
        ...
        const loadAttribute = async () => {
            ...
            try {
                ...
                form.attribute_set_id = data.attribute_set_id || '';
                form.name = data.name || '';
                form.categories = data.categories || [];
                form.is_filterable = data.is_filterable || false;
                form.values = data.values && data.values.length > 0 
                    ? data.values 
                    : [{ value: '' }];
            } catch (error) {
        ...
        const handleSubmit = async () => {
            ...
            try {
                const data = {
                    ...
                    values: form.values.filter(v => v.value.trim() !== '')
                };
                ...
                notification.error('Error', 'Failed to save attribute: ' + (error.response?.data?.message || error.message));
            } finally {
                loading.value = false;
            }
        };
