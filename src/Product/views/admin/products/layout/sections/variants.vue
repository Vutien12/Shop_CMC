                                    ...
                                    >
                                        <div class="d-flex align-items-center">
                                            ...
                                            <ul class="variant-badge list-inline d-flex">
                                                ...
                                                <li v-if="variant.is_active && (variant.in_stock == 0 || (variant.manage_stock && Number(variant.qty) == 0))">
                                            ...
                                            >
                                                <div class="media-grid-item handle" v-for="(media, mediaIndex) in variant.media" :key="mediaIndex">
                                                    <div class="image-holder">
                                                        ...
                                                        <button type="button" class="btn remove-image" @click="removeVariantMedia(index, mediaIndex)">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                                ...
                                                <div class="media-grid-item media-picker disabled" @click="addVariantMedia(index)">
                                                    <div class="image-holder">
                                                        <img :src="placeholderImage" class="placeholder-image" alt="Placeholder image">
                                                    </div>
                                                </div>
                                            </draggable>
                                        </div>
...
export default {
    ...
    data() {
        return {
            isCollapsedVariantsAccordion: false,
        };
    },
    methods: {
        trans(key) {
        ...
        addVariantMedia(variantIndex) {
            this.$emit('add-variant-media', variantIndex);
        },

        preventLastSlideDrag(event) {
            const mediaCount = event.from.querySelectorAll('.media-grid-item').length;
            return event.draggedContext.index < mediaCount - 1;
        },

        removeVariantDatePickerValue(index, field) {
            this.form.variants[index][field] = '';
        },
